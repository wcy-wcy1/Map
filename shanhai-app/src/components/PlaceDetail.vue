...
const memoryIntro = computed(() => props.visits.length
  ? `${props.visits.length} 段回忆留在这里，最近一次是 ${props.visits[0]?.date ?? ''}。`
  : '这里还没有你的旅行回忆。')
</script>
...
<template>
  <section class="yn-detail" tabindex="-1" aria-live="polite">
    ...
    <div class="yn-memory-album-heading">
      <div>
        <h3>在这里的回忆 <small>{{ visits.length }} 段</small></h3>
        <p>{{ memoryIntro }}</p>
      </div>
      <button type="button" class="yn-primary" :disabled="disabled" @click="emit('add')">{{ visits.length ? '再记一次' : '记下这次到访' }}</button>
    </div>
    ...
    <article v-for="visit in page.items" :key="visit.id" class="yn-memory">
      <p class="yn-memory-meta"><time :datetime="visit.date">{{ visit.date }}</time><span v-if="visit.photos.length">{{ visit.photos.length }} 张照片</span></p>
      <div class="yn-memory-body" :data-has-photo="!!visit.photos[coverIndex(visit)]">
        ...
        <p v-else class="yn-memory-note yn-memory-note-empty">那天只留下了照片。</p>
      </div>
